...
import { Profile } from '../data/network';
    ...
    return (
        <div className="bg-brand-gray border border-gray-700 rounded-lg p-4 sm:p-6 text-center group hover:border-green-500 transition-all duration-300 transform hover:-translate-y-1 flex flex-col h-full">
            <img 
                ...
                className="w-20 h-20 sm:w-24 sm:h-24 rounded-full mx-auto mb-4 border-4 border-gray-600 group-hover:border-green-500 transition-colors"
            />
            <div className="flex-grow">
                <h3 className="text-lg sm:text-xl font-bold text-white">{profile.name}</h3>
                <p className="text-sm font-semibold text-green-500 mb-3">{profile.category}</p>
                <p className="text-gray-400 text-sm">{profile.tagline}</p>
            </div>
            <a 
